...
export async function logAdminActivity(user, ipAddress, userAgent, options) {
    try {
        ...
        if (config.auditLog) {
            ...
            console.log(`[AUDIT] ${options.action} by ${user.email} (${user.id}) from ${ipAddress}`, options);
        }
    }
    catch (error) {
...
export async function getAdminActivitiesByResource(resourceType, resourceId) {
    ...
    for (const date of availableDates.slice(0, 30)) {
        ...
        const filtered = logs.filter((log) => log.metadata?.resourceType === resourceType &&
            log.metadata?.resourceId === resourceId);
        activities.push(...filtered.map((log) => ({
            id: `${log.timestamp}-${log.userId}`,
            admin_user_id: log.userId,
            admin_email: log.metadata?.adminEmail,
            action: log.message,
    ...
    return activities.sort((a, b) => new Date(b.created_at).getTime() -
...
export const adminFileLogger = {
    log: async (options) => {
        try {
            const config = getLoggingConfig();
            if (config.auditLog) {
                await config.auditLog(options.actionDescription, options.adminId, 'system', {
                ...
                console.log(`[AUDIT] ${options.actionDescription} by ${options.adminId}`, options);
            }
        }
        catch (error) {
